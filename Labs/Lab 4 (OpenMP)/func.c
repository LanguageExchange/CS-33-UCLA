...
{
	int i;
	#pragma omp parallel for firstprivate(n, arrayX, arrayY, weights, xr, yr) private(i)
	for(i = 0; i < n; i++){
			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	#pragma omp parallel for firstprivate(arrayX, arrayY, n, seed) private(i)
   	for(i = 0; i < n; i++){
   		arrayX[i] += 1 + 5*rand2(seed, i);
   		arrayY[i] += -2 + 2*rand2(seed, i);
   	}
	
	#pragma omp parallel for firstprivate(Ones, index_X, index_Y, Y, Z, arrayX, arrayY) private(i, j)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
...
void func2(double *weights, double *probability, int n)
{
	...
	//#pragma omp parallel for firstprivate(n, weights, probability) private(i)
	for(i = 0; i < n; i++)
   		weights[i] = weights[i] * exp(probability[i]);
	
	//#pragma omp parallel for firstprivate(n, weights) private(i)
   	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
	...
	//#pragma omp parallel for firstprivate(n, arrayX, arrayY, weights, estimate_x, estimate_y) private(i)
	for(i = 0; i < n; i++){
...
void func4(double *u, double u1, int n)
{
	...
	#pragma omp parallel for firstprivate(n, u, u1) private(i)
	for(i = 0; i < n; i++){
   		u[i] = u1 + i/((double)(n));
   	}
}
...
void func5(double *x_j, double *y_j, double *arrayX, double *arrayY, double *weights, double *cfd, double *u, int n)
{
	...
	#pragma omp parallel for firstprivate(n, cfd, u, i, x_j, y_j, arrayX, arrayY) private(j)
	for(j = 0; j < n; j++){
	...
	#pragma omp parallel for firstprivate(n, arrayX, arrayY, x_j, y_j, weights) private(i)
	for(i = 0; i < n; i++){
		arrayX[i] = x_j[i];
		arrayY[i] = y_j[i];
		weights[i] = 1/((double)(n));
